...
// ========================= [BEGIN TmBot3000::TimeTerms Routing Block] =========================
// This block is self-contained. It:
// - Loads authoritative time-term map from DB view tm_time_terms
// - Detects time phrasing
// - Fast-path by term_id (intent.entities.term_id) → field_key/label
// - NL fallback (term alias → field_key) using DB labels and show keys
// - Returns schedule text when a city + value is found; else friendly fallback
//
// Requirements assumed available elsewhere in this file:
// - an async getNextShowByCity(city)
// - a logger (console.* is fine)
// - generateResponse(params) dispatches for intent.intent_type === 'term_lookup'
// -----------------------------------------------------------------------------------------------

// OPTIONAL: Use existing pool if present; otherwise create a minimal one.
let __tt_pool = global.__tt_pool || null;
if (!__tt_pool) {
  try {
    const { Pool } = require('pg');
    __tt_pool = new Pool({
    ...
    global.__tt_pool = __tt_pool;
  } catch (e) {
    console.warn('[TimeTerms] pg Pool not initialised in tmAiEngine.js — ensure a Pool is imported or the optional bootstrap above is enabled.');
  }
}

// In-memory cache of DB view: term_id → { field_key, label }
const __timeTermMap = new Map();
// Lowercased alias → field_key
const __timeAliases = new Map();
// Guard to avoid concurrent loads
let __timeTermsLoading = null;

// Liberal time-phrasing detector
const __TIME_PHRASE_RX = new RegExp(
  [
    '\\bwhat\\s*time\\b',
    '\\bwhen\\b',
    '\\bwhen\\s*do\\b',
    '\\bwhen\\s*does\\b',
    '\\bdoors?\\s*(open|time)?\\b',
    '\\b(load\\s*-?in|load\\s*-?out)\\b',
    '\\bcurfew\\b',
    '\\bon\\s*-?stage\\b',
    '\\bshow\\s*time\\b',
    '\\bsound\\s*check|soundcheck\\b',
    '\\bcrew\\s*call\\b',
    '\\bband\\s*call\\b',
  ].join('|'),
  'i'
...
// Built-in alias seeds
const __BUILTIN_TIME_ALIASES = {
  'doors': 'doors_time',
  'doors open': 'doors_time',
  'doors time': 'doors_time',
  'soundcheck': 'soundcheck_time',
  'sound check': 'soundcheck_time',
  ...
  'show time': 'show_time',
  'showtime': 'show_time',
  'crew call': 'crew_call_time',
  'band call': 'band_call_time',
};

function __isTimePhrase(q) {
  return __TIME_PHRASE_RX.test(q);
}
...
function __lc(s) {
  return __normalize(s).toLowerCase();
}

function __prettifyLabelFromField(field) {
  return (__normalize(field).replace(/_/g, ' ').replace(/\b\w/g, m => m.toUpperCase()));
}

async function __loadTimeTermsFromDb() {
  if (__timeTermsLoading) return __timeTermsLoading;
  __timeTermsLoading = (async () => {
    __timeTermMap.clear();
    __timeAliases.clear();

    // Seed with built-ins
    for (const [k, v] of Object.entries(__BUILTIN_TIME_ALIASES)) {
      __timeAliases.set(__lc(k), v);
    }

    if (!__tt_pool) {
      console.error('[TimeTerms] No DB pool available — cannot load tm_time_terms. Fast-path disabled.');
      return;
    }

    const sql = `
      SELECT term_id, field_key, label
      FROM tm_time_terms
      WHERE field_key IS NOT NULL AND label IS NOT NULL
    `;
    const res = await __tt_pool.query(sql);
    for (const row of res.rows) {
      const term_id = __normalize(row.term_id);
      const field_key = __normalize(row.field_key);
      const label = __normalize(row.label);

      if (term_id && field_key) {
        __timeTermMap.set(term_id, { field_key, label });
        const base = __lc(label);
        __timeAliases.set(base, field_key);
        __timeAliases.set(base.replace(/\stime$/, ''), field_key);
        __timeAliases.set(base.replace(/\s+/, ' '), field_key);
      }
    }
    console.info(`[TimeTerms] Loaded ${res.rowCount} terms from tm_time_terms.`);
  })().finally(() => {
    __timeTermsLoading = null;
  });

  return __timeTermsLoading;
}

async function __resolveByTermId(termId) {
  if (!__timeTermMap.size) {
    await __loadTimeTermsFromDb();
  }
  return __timeTermMap.get(termId) || null;
}

function __stripStops(s) {
  const stop = /\b(what|when|time|is|are|do|does|the|a|an|in|at|for|of|to|open|up|on|stage)\b/gi;
  return __normalize(s).replace(stop, ' ').replace(/\s+/g, ' ').trim();
}

function __resolveFieldFromText(q, showObj) {
  const cleaned = __stripStops(q.toLowerCase());
  const aliasKeys = Array.from(__timeAliases.keys()).sort((a, b) => b.length - a.length);
  for (const a of aliasKeys) {
    if (cleaned.includes(a)) return __timeAliases.get(a);
  }
  if (showObj && typeof showObj === 'object') {
    const keys = Object.keys(showObj);
    const timeKeys = keys.filter(k => /_time$/.test(k));
    for (const k of timeKeys) {
      const hint = k.replace(/_time$/, '').replace(/_/g, ' ');
      if (cleaned.includes(hint)) return k;
    }
  }
  return null;
}

function __renderScheduleLine({ label, city, venue, date, value, tz }) {
  const lbl = label || __prettifyLabelFromField('time');
  const sz = tz ? ` ${tz}` : '';
  return `${lbl} for ${city} (${venue}) on ${date}: ${value}${sz}`;
}

function __friendlyMissingCity() {
  return `I can grab the exact time if you tell me the city (e.g., “when are doors in Sydney?”).`;
}

function __friendlyMissingField(label, city) {
  const what = label ? label : 'that time';
  return `I couldn’t find ${what} for ${city} on the next show. If there’s a later date or a different city, try that.`;
}

async function __handleTimeTermRouting({ text, intent, parseCityAndTerm, getNextShowByCity }) {
  const q = __normalize(text);
  const isTimey = __isTimePhrase(q);

  if (!isTimey) return null;

  // 1) DB fast-path
  const termId = intent?.entities?.term_id;
  if (termId) {
    const hit = await __resolveByTermId(termId);
    if (hit) {
      const { city } = parseCityAndTerm(q) || {};
      if (!city) {
        return { type: 'fallback', text: __friendlyMissingCity() };
      }
      const show = await getNextShowByCity(city);
      if (show && show[hit.field_key]) {
        const label = hit.label || __prettifyLabelFromField(hit.field_key);
        return {
          type: 'schedule',
          text: __renderScheduleLine({
            label,
            ...
            venue: show.venue_name || 'TBA',
            date: show.date || 'TBA',
            value: show[hit.field_key],
            tz: show.timezone || '',
          }),
        };
      } else {
        return { type: 'fallback', text: __friendlyMissingField(hit.label, city) };
      }
    }
  }

  // 2) NL fallback
  const parsed = parseCityAndTerm(q) || {};
  ...
  const show = await getNextShowByCity(city);
  if (!show) {
    return { type: 'fallback', text: `I couldn’t find an upcoming show for ${city}. Try another city?` };
  }

  const fieldKey = __resolveFieldFromText(q, show);
  if (fieldKey && show[fieldKey]) {
    let label = null;
    if (!__timeTermMap.size) await __loadTimeTermsFromDb();
    for (const { field_key, label: L } of __timeTermMap.values()) {
      if (field_key === fieldKey) { label = L; break; }
    }
    label = label || __prettifyLabelFromField(fieldKey);
    return {
      type: 'schedule',
      text: __renderScheduleLine({
        label,
  ...
  return { type: 'fallback', text: __friendlyMissingField(parsed.term || 'that time', city) };
}
// ========================== [END TmBot3000::TimeTerms Routing Block] ==========================
